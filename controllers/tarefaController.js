...
function addTarefa(req, res) {
  ...
  Tarefa.create(titulo, descricao, status,(err, id) => {
    if (err) {
...
function updateTarefa(req, res) {
  ...
  const { titulo, descricao } = req.body; // Obtém os dados atualizados do formulário

  // Faça uma consulta para atualizar os dados da tarefa no banco de dados
  Tarefa.update(id, titulo, descricao, (err) => {
    if (err) {
